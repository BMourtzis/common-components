    ...
    <v-layout wrap>
      <template v-for="(field, fieldIndex) of schema">
        ...
        <v-flex
          ...
          sm6
          :md[field]="true"
          :md3="field.smallQuarter"
          :md6="field.half"
          :md9="field.largeQuarter"
          :md12="field.full"
        >
          <v-text-field
        ...
        <v-spacer :key="fieldIndex" v-else-if="field.type === SPACER"></v-spacer>
        <v-flex :key="fieldIndex" v-else-if="field.type === SUBHEADING" xs12 sm12 md12>
          <span class="subheading">{{$t(field.label)}}</span>
        </v-flex>
        <template v-else-if="field.type === LIST">
          <template v-for="(item, index) of model[field.name]">
            <v-flex
              v-for="(subfield, subIndex) of field.fields"
              :key="(index * 2) + subIndex"
              xs12
              sm6
              :md4="subfield.small"
              :md5="subfield.large"
